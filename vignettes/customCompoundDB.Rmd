...
databases by providing ChEMBL ids of the query compounds.
...
The following shows functions used to add user's custom compound annotation 
...
and `LINCS`. The detailed description of this SQLite database is available at the
...
of the `compoundCollectionData` package. The custom compound 
annotations could be added/deleted as described above. The input of the query 
function is a set of ChEMBL IDs, it returns a data.frame storing annotations 
of the input compounds from the selected annotation resources defined by the 
\code{annot} argument.
